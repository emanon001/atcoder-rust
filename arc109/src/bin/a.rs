...
fn solve() {
    input! {
        a: usize, b: usize, x: i64, y: i64
    };

    let mut graph = Graph::new(vec![], 201, 1_i64 << 60);
    for i in 1..=100 {
        graph.add_edge((i, i + 100, x));
    }
    for i in 1..=99 {
        graph.add_edge((i + 1, i + 100, x));
    }
    for i in 1..=99 {
        graph.add_edge((i, i + 1, y));
        graph.add_edge((i + 100, i + 101, y));
    }
    let res = graph.shortest_path(a)[b + 100].unwrap();
    println!("{}", res);
}
