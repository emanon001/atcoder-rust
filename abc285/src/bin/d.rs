...
fn solve() {
    ...
    let zahyo = compress_zahyo(&words);
    let edges = stv
        .into_iter()
        .map(|(s, t)| (zahyo[&s], zahyo[&t]))
        .collect::<Vec<_>>();
    let graph = Graph::new_directed(edges, zahyo.len(), 1_i64 << 60);
    let res = if graph.scc().into_iter().any(|g| g.len() > 1) {
        "No"
    } else {
        "Yes"
    };
