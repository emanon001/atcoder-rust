...
fn solve() {
    ...
    let mut st: SegmentTree<i64> = SegmentTree::new(n);
    st.update(0, 0);
    for i in 1..n {
        let x = xv[i];
        let range = (x - r)..=(x - l);
        let l = set.range(range.clone()).next();
        let r = set.range(range).next_back();
        match (l, r) {
            (Some((_, l)), Some((_, r))) => {
                let step = st.query(*l, *r + 1) + 1;
                st.update(i, step);
            }
            _ => {}
        }
    }
    println!("{}", st.query(n - 1, n));
}
