...
fn solve() {
    ...
    let inf = 1_i64 << 60;
    let mut self_edge = vec![None; n];
    let mut new_edges = Vec::new();
    for (u, v, c) in edges {
        if u == v {
            if self_edge[u].is_none() {
                self_edge[u] = Some(c);
            } else if c < self_edge[u].unwrap() {
                self_edge[u] = Some(c);
            }
            continue;
        }
        new_edges.push((u, v, c));
    }

    let graph = Graph::new_directed(new_edges, n, inf);
    let mut shortest_paths = vec![Vec::new(); n];
    for i in 0..n {
        shortest_paths[i] = graph.shortest_path(i);
    }
    for i in 0..n {
        let a = &shortest_paths[i];
        let mut res = inf;
        if self_edge[i].is_some() {
            res = self_edge[i].unwrap();
        }
        for j in 0..n {
            if i == j {
                continue;
            ...
            let b = &shortest_paths[j];
            if b[i].is_none() {
                continue;
            }
            let cost = a[j].unwrap() + b[i].unwrap();
            if cost < res {
                res = cost;
            }
        }
