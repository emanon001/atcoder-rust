...
where
    T: PartialEq + Eq + Copy,
{
    pub fn new(grid: Vec<Vec<T>>, ng: Option<T>) -> Self {
        assert!(grid.len() > 0);
        let h = grid.len();
        let w = grid[0].len();
        Self { grid, h, w, ng }
...
fn main() {
    ...
    let graph = grid.to_graph(|grid, i, j| {
        ...
        for &(di, dj) in &UDLR_DIRS {
            ...
            let new_i = new_i as usize;
            let new_j = new_j as usize;
            dest.push(((new_i, new_j), grid.cell(new_i, new_j)));
        }
        dest
    });
    let mut res = WeightedGraph::INF;
    let start = grid.vertex(h - 1, 0);
    let midpoint = grid.vertex(h - 1, w - 1);
    let goal = grid.vertex(0, w - 1);
    for i in 0..h {
        for j in 0..w {
            let u = grid.vertex(i, j);
            let d = graph.shortest_path(u);
            let cost =
                grid.cell(i, j) + d[start].unwrap() + d[midpoint].unwrap() + d[goal].unwrap();
            res = res.min(cost);
        }
    }
    println!("{}", res);
