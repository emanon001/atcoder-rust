...
fn solve() {
    ...
    for i in 0..n {
        for j in 0..n {
            ...
            edges2.push((i, j, costs[i][j] * b + c));
        }
    }

    let g1 = Graph::new_undirected(edges1, n, 1_i64 << 60);
    let g2 = Graph::new_undirected(edges2, n, 1_i64 << 60);
    let d1 = g1.shortest_path(0);
    let d2 = g2.shortest_path(n - 1);
    let mut ans = 1_i64 << 60;
    for u in 0..n {
        if let (Some(c1), Some(c2)) = (d1[u], d2[u]) {
            let cost = c1 + c2;
            chmin!(ans, cost);
        }
    }
