...
fn dfs(
    ...
    if step == step_pos.len() - 1 {
        let res = 0;
        dp[u] = Some(res);
        return res;
    }

    let mut res = WeightedGraph::INF;
    let dist = graph.shortest_path_1(u);
    for &v in &step_pos[step + 1] {
        if let Some(w) = dist[v] {
            res = res.min(dfs(step + 1, v, step_pos, graph, dp) + w);
        }
    }
    dp[u] = Some(res);
    res
...
fn solve() {
    ...
    let grid = Grid::new(&grid, '_');
    let graph = grid.to_graph(|grid, i, j| gen_grid_destinations(grid, i, j, &UDLR_DIRS));
    // 0: 'S', n + 1: 'G'
    let mut step_pos = vec![Vec::new(); 9 + 2];
    for i in 0..n {
        for j in 0..m {
            let step = match grid.cell(i, j) {
                'S' => 0,
                'G' => 10,
                ch => ch.to_digit(10).unwrap() as usize,
            };
            step_pos[step].push(grid.vertex(i, j));
        }
    }
    let mut res = WeightedGraph::INF;
    let mut dp = vec![None; graph.vn];
    for &u in &step_pos[0] {
        res = res.min(dfs(0, u, &step_pos, &graph, &mut dp));
    }
    let res = if res == WeightedGraph::INF { -1 } else { res };
    println!("{}", res);
}
